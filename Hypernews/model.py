...
word2vec_path = '../dataset/Hypernews/content_data/word2vec_wv.model'
doc2vec_path = '../dataset/Hypernews/content_data/doc2vec.npy'
doc_topics_path = '../dataset/Hypernews/content_data/doc_topics.npy'
...
class MODEL(object):
    ...
    def _build_model(self, args):
        with tf.name_scope('embeddings',):
            ...
            category_embedding = tf.Variable(tf.random_normal((260, 100), stddev=1), trainable=True, name='category_embedding')
            self.category_embeddings = tf.concat([zero_category_embedding, category_embedding], axis=0)

            #zero_topic_embedding=tf.zeros(shape=[1,100],dtype=tf.float32)
            self.topic_embeddings = tf.Variable(topic_emb, dtype=tf.float32, trainable=False, name='topic_embedding')
            #self.topic_embeddings=tf.concat([zero_topic_embedding,topic_embedding],axis=0)

            self.city_embeddings = tf.Variable(tf.random_normal((5050, 50), stddev=1), trainable=True, name='city_embedding')
            self.region_embeddings = tf.Variable(tf.random_normal((1100, 50), stddev=1), trainable=True, name='region_embedding')
            self.age_embeddings = tf.Variable(tf.random_uniform(shape=[90, 100], minval=-1, maxval=1), trainable=True, name='age_embedding')

            zero_len_embedding = tf.zeros(shape=[1, 50], dtype=tf.float32)
            len_embedding = tf.Variable(tf.random_uniform(shape=[1000, 50], minval=-1, maxval=1), trainable=True, name='len_embedding')
        ...
        self.unnormized_p = tf.reshape(tf.diag_part(tf.matmul(news_embedding_p, user_embedding, transpose_b=True)), shape=[-1, 1])
    ...
    # def _attention_p(self, args, explicit_emb, implicit_emb):
    #     with tf.variable_scope('attention', reuse=tf.AUTO_REUSE):
    #         attention_w = tf.get_variable(name='attention_p_w', shape=[200, 200], dtype=tf.float32, trainable=True)
    #         attention_b = tf.get_variable(name='attention_p_b', dtype=tf.float32, shape=[200, 1], trainable=True)
    #         attention_q = tf.get_variable(name='attention_p_q', dtype=tf.float32, shape=[1, 200], trainable=True)
    ...
    #     norminized_att = tf.nn.softmax(att, axis=1)
    #
    #     self.att_p = tf.reduce_mean(norminized_att, 0)
    #     self.att_t = tf.reduce_mean(norminized_att, 0)
    #     self.att_u = tf.reduce_mean(norminized_att, 0)
    #
    #     news_emb = tf.multiply(explicit_emb, tf.slice(norminized_att, [0, 0], [-1, 1]))+tf.multiply(implicit_emb, tf.slice(norminized_att, [0, 1], [-1, 1]))
    #     return news_emb
    ...
    def _user_encoder(self, args):
        ...
        with tf.variable_scope('self_attention', reuse=tf.AUTO_REUSE):
            ...
            for i in range(args.max_click_history):
                alpha_i = tf.matmul(self_q, tf.matmul(self_w, tf.reshape(news_emb[:, i, :], shape=[-1, 200]), transpose_b=True) + self_b)
                alpha.append(alpha_i)
            alpha = tf.nn.softmax(tf.transpose(alpha), axis=-1)
        clicked_sum = tf.reduce_sum(tf.multiply(news_emb, tf.reshape(alpha, shape=[-1, args.max_click_history, 1])), axis=1)
    ...
    def test(self, sess, feed_dict):
        loss, time_loss, labels, scores, time_pre, time_true, time_label = sess.run([self.loss, self.classify_loss, self.labels, self.p, self.time_pre, self.time_true, self.time_label], feed_dict)
