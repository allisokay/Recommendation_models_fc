...
def load_data(args, load_file):
    ...
    with open(load_file, 'r', encoding='utf-8') as f:
        ...
        for line in lines:
            ...
            if line_list[0] not in user_id_to_history.keys():
                continue
            if len(user_id_to_history[line_list[0]]) == 0:
                continue
            user_id.append(line_list[0])
            news_id.append(line_list[1])
            time.append(int(line_list[2]))
            active_time.append(int(line_list[3]))
